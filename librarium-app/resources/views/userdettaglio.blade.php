            ...
            <div class="glass-effect-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <div class=" d-flex flex-column align-items-center">
                    ...
                    <form method="POST" action="{{route('user.destroy',['user' => $user])}}">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Sei sicuro di voler eliminare?')" class="btn btn-customG" style="width: 200px;">Elimina</button>
                    </form>
                </div>
            </div>
        </div>
...
<!-- <ul class="list-group">
    ...
    <li class="list-group-item">
        ...
        <form method="POST" action="{{route('user.destroy',['user' => $user])}}">
            @csrf
            @method('DELETE')
            <button type="submit" onclick="return confirm('Sei sicuro di voler eliminare?')" class="btn btn-outline-danger">Elimina</button></a>
        </form>
    </li>
</ul> -->
</div>
